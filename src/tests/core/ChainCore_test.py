...
class TestChainManager:
    ...
    def chain_manager(self):
        ...
        with patch("src.core.ChainCore.SystemConfig"):
            return ChainManager()
    ...
    def test_remove_chain(self, chain_manager, task_chain_configs):
        ...
        assert len(chain_manager.chains) == 3

        # Remove a chain
        chain_manager.remove_chain("chain2")
    ...
    def test_remove_nonexistent_chain(self, chain_manager):
        ...
        # Try to remove a nonexistent chain
        chain_manager.remove_chain("nonexistent-chain")
...
class TestChainExecutor:
    ...
    def test_execute_chain_validation_failure(self, chain_executor, task_chain_config):
        ...
        assert "failed validation" in result.error.lower()
    ...
    def test_execute_chain_resource_allocation_failure(
        ...
        assert isinstance(result, ExecutionResult)
        assert result.success is False
        assert "resource allocation" in result.error.lower()
